...
# INITIAL VERSION - 0.1, Oct 30 2011
...
def main():
    ...
    print("")
    print("================================")
    print("Sage to Debian packaging utility")
    print("================================")
    print("")
    ...
    if f_spkg_install != None:
        ...
        if f_configure != None:
            ...
            try:
                ...
                generate_deb_from_source(working_dir, working_dir_project,
                                    src_path, f_setup_py)
            except:
...
def generate_src_deb_from_source(working_dir, working_dir_project,
                                src_path, f_setup_py):
    generate_deb_from_source(working_dir, working_dir_project,
                                src_path, f_setup_py, True)


def generate_deb_from_source(working_dir, working_dir_project,
                            src_path, f_setup_py, build_source_only=False):
    debian_dir = os.path.join(src_path, "debian")
    process_command(["mkdir", "-p", debian_dir], cwd=src_path)
    tpl(os.path.join(spkg2deb_home, "control.spkg.tpl"),
    ...
    if build_source_only == True:
        ...
        try:
            ...
            build_source_only = True
            print("....making binaries failed! (bad Makefile or missing deps)")
            print("....building deb package with sources only...")
            process_command_quiet(["dpkg-buildpackage", "-rfakeroot", "-uc",
                                "-us", "-S"], cwd=src_path)
    try:
        if build_source_only == True:
            deb_file = PACKAGE_NAME + "_" + VERSION + ".tar.gz"
            build_text = "....created sources package only: "
        else:
            deb_file = PACKAGE_NAME + "_" + VERSION + "_all.deb"
            build_text = "....created binary and sources package: "
        ...
        process_command(["cp",
            os.path.abspath(deb_file), femhub_deb], cwd=working_dir)
        os.chmod(os.path.join(femhub_deb, deb_file), 0664)
...
def generate_deb_from_python(working_dir, working_dir_project,
    ...
    try:
        ...
        # delete .pyc from project recursively or it won't build!!!
        ...
        if (os.path.exists(os.path.join(f_setup_py_path, "deb_dist"))):
            try:
                ...
                process_command(["cp",
                    ...
                    f_setup_py_path, "deb_dist", deb_file)), femhub_deb],
                    cwd=f_setup_py_path)
                os.chmod(os.path.join(femhub_deb, deb_file), 0664)
